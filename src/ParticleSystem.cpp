...
void ImpulseRadialForce::updateForce(Particle *particle) {
	...
	//
	ofVec3f dir = ofVec3f(ofRandom(-100, 100), ofRandom(-100, 100), ofRandom(-100, 100));
	ofVec3f temp = dir.getNormalized() * magnitude;
	if (heightLimit) {
		if (particle->forces.y + temp.y >= height)
		{
			temp.y = 0;
		}
		else if (particle->forces.y + temp.y <= -height)
		{
			temp.y = 0;
		}
	}

	particle->forces += temp;
}
...
void ImpulseRadialForce::setheightLimit(bool tf)
{
	this->heightLimit = tf;
}